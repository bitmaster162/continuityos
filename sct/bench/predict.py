...
def validate_probability_response(options: Sequence[str], response: Mapping[str,Any]) -> tuple[dict[str,float], str, float]:
    opts=tuple(options)
    ...
    predicted=min(opt for opt,p in clean.items() if abs(p-maxp)<=1e-15)
...
def build_prediction(*, case_id: str, arm: str, options: Sequence[str], response: Mapping[str,Any], committed_at: float) -> Prediction:
    ...
    if not isinstance(escalate,bool): raise BenchError("PREDICTION_SCHEMA_VIOLATION: would_escalate must be boolean")
    body={"schema":PREDICTION_SCHEMA,"case_id":case_id,"arm":arm,"options":tuple(options),
          "option_probabilities":probs,"predicted_choice":pred,"confidence":conf,"reasons":reasons,
          "change_conditions":changes,"would_escalate":escalate,"committed_at":float(committed_at),
          "execution_authority":"NONE","can_execute":False}
    return Prediction(prediction_id=sha256_obj(body), **{k:body[k] for k in (
