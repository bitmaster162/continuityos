param(
    [string]$TargetSha = "670d83f13f23dd9a3294015c9cffa774adc043ef"
)
...
$ExpectedPrior = "41fbbbddfdd36865ad9d661f11fc19d5babf5459"
...
function Assert-HumanUiContract {
    ...
    Require ($html.Contains('DEEP-LITE')) "UI missing DEEP-LITE"
    Require ($html.Contains('/ask/deep-lite')) "UI missing dedicated DEEP-LITE endpoint"
    Require ($html.Contains('id="answer"')) "UI missing human-readable answer target"
    Require ($html.Contains('id="evidence"')) "UI missing evidence list"
    Require ($html.Contains('Evidence')) "UI missing Evidence label"
    Require ($html.Contains('Raw response')) "UI missing raw-response disclosure"
    Require ($html.Contains('LOCAL_SHADOW')) "UI missing LOCAL_SHADOW boundary"
    Require ($html.Contains('AUTHORITY NONE')) "UI missing authority boundary"
    Require ($html.Contains('renderAnswer')) "UI missing human-answer renderer"
    Require ($html.Contains('textContent')) "UI missing safe text rendering"
    Require (-not $html.Contains('innerHTML')) "UI contains forbidden innerHTML sink"
    Require ($html.Contains('Query required.')) "UI missing empty-query state"
    Require ($html.Contains('No answer text returned.')) "UI missing empty-answer state"

    Write-Host "R15_HUMAN_UI_CONTRACT=PASS"
}
...
$transcript = Join-Path $Receipts "dogfood-r15-ui-$stamp.log"
$work = Join-Path $env:TEMP ("sovereign-twin-r15-ui-" + [guid]::NewGuid().ToString('N'))
...
try {
    Step 'Preflight live R14 runtime and authority'
    ...
    if ($oldSha -ne $TargetSha) {
        Step 'Prebuild exact R15 target and R14 rollback wheels'
        ...
        Step 'Stop validated Twin and install exact R15 candidate'
    ...
    Step 'Verify R15 source receipt and human-readable UI contract'
    $postManifest = Get-Content -LiteralPath $ManifestPath -Raw | ConvertFrom-Json
    Require ([string]$postManifest.source_sha -eq $TargetSha) "manifest source SHA is not R15 target"
    Require ((FullPath ([string]$postManifest.memory_db)) -eq (FullPath $activeDb)) "post-install manifest DB pointer changed"
    Assert-HumanUiContract

    Assert-LmsEmpty 'PRE_API_DOGFOOD'

    Step 'Run real DEEP-LITE through unchanged loopback HTTP API'
    ...
    $receipt = [ordered]@{
        schema = 'sovereign-twin.dogfood-r15-ui/v1'
        ok = $true
        source_sha = $TargetSha
        prior_source_sha = $oldSha
        endpoint = '/ask/deep-lite'
        ui_contract = 'human-readable-safe-text'
        active_db = $activeDb
    ...
    $receiptPath = Join-Path $Receipts "dogfood-r15-ui-$stamp.json"
    $receipt | ConvertTo-Json -Depth 20 | Set-Content -LiteralPath $receiptPath -Encoding UTF8

    Step 'PASS'
    Write-Host 'R15_UI_DOGFOOD=PASS' -ForegroundColor Green
    Write-Host "SOURCE_SHA=$TargetSha"
    Write-Host 'HUMAN_UI_CONTRACT=PASS'
    ...
    Write-Host "`nR15_UI_DOGFOOD=FAIL" -ForegroundColor Red
    ...
    if ($targetInstallAttempted -and $rollbackWheel -and $oldSha -and $activeDb) {
        try {
            ...
            [void](Start-And-VerifyTwin $activeDb)
            Write-Host "ROLLBACK=PASS source_sha=$oldSha"
        } catch {
            Write-Host "ROLLBACK=FAIL $($_.Exception.Message)" -ForegroundColor Red
        }
    } elseif ($activeDb) {
        try {
            [void](Ensure-TwinRunning $activeDb)
        } catch {
            Write-Host "RECOVERY=FAIL $($_.Exception.Message)" -ForegroundColor Red
        }
    }
}
finally {
