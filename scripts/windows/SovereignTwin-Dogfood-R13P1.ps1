...
try {
    ...
    Step 'Verify installed source receipt and Windows-safe JSON emitter'
    $postManifest = Get-Content -LiteralPath $ManifestPath -Raw | ConvertFrom-Json
    Require ([string]$postManifest.source_sha -eq $TargetSha) "manifest source SHA is not target"
    ...
    $stderrPath = Join-Path $work 'deep-lite-stderr.txt'

    $sw = [Diagnostics.Stopwatch]::StartNew()
    $raw = (& $Py -m continuityos.sovereign_twin_deep_lite `
        $query `
        --db $activeDb `
        --embedding-model $EmbeddingModel `
        --model 'qwen3.5-4b' `
        2> $stderrPath | Out-String).Trim()
    ...
    if ($exitCode -ne 0) {
        Write-Host "DOGFOOD_STDOUT=$raw"
        if ($stderrRaw) { Write-Host "DOGFOOD_STDERR=$stderrRaw" }
    }
    Require ($exitCode -eq 0) "DEEP-LITE exited ${exitCode}"
    Require (-not [string]::IsNullOrWhiteSpace($raw)) 'DEEP-LITE returned empty stdout'
    ...
    $receipt = [ordered]@{
        schema = 'sovereign-twin.dogfood-r13p1/v2'
        ok = $true
