"""Verified-current-session CLI for read-only R38 bootstrap preflight."""
...
from .current_bootstrap_check import CHECK_SCHEMA, check_project_memory_bootstrap
from .current_effect_boundary import MODE_CURRENT, inspect_current_session

CLI_SCHEMA = "continuityos.current_bootstrap_check.cli/v1"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="continuity-memory-bootstrap-check",
        description=(
            "Validate an exact R38 manifest, authorization record, evidence set and target "
            "without applying bootstrap. Requires a verified current session."
        ),
    )
    ...
    if state.get("mode") != MODE_CURRENT or state.get("binding_verified") is not True:
        _emit({
            "schema": CLI_SCHEMA,
            "terminal": "CURRENT_BOOTSTRAP_CHECK_REVISE",
    ...
    result = check_project_memory_bootstrap(args.db, args.manifest, args.authorization)
