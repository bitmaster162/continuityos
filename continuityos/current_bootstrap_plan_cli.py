"""Verified current-session CLI for proposal-only project bootstrap manifests."""
...
from .current_bootstrap_plan import PLAN_SCHEMA, REQUEST_SCHEMA, build_bootstrap_plan
...
CLI_SCHEMA = "continuityos.current_bootstrap_plan.cli/v1"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="continuity-memory-bootstrap-plan",
        description=(
            "Compile a deterministic R38 bootstrap manifest proposal from exact local evidence. "
            "Requires a verified current session; never authorizes or applies bootstrap."
        ),
    )
    ...
    try:
        payload = _stable_read(request_path, "bootstrap-plan request", max_bytes=MAX_ARTIFACT_BYTES)
        request = strict_json_loads(payload.decode("utf-8-sig"))
    except Exception as exc:
        _emit(_revise(
            "BOOTSTRAP_PLAN_REQUEST_UNREADABLE",
            current_session={
    ...
    _emit(result)
    return 0 if result.get("schema") == PLAN_SCHEMA and result.get("terminal") == "CURRENT_BOOTSTRAP_PLAN_PASS" else 2
