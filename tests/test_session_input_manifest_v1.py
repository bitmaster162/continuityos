...
from continuityos.operational_context import prepare_context_pack, verify_context_pack
...
def make_inputs(tmp_path: Path, *, capsule_value=None, spec_value=None):
    ...
    verify_path = tmp_path / "OPERATIONAL_CONTEXT_VERIFY_RECEIPT.json"
    verify_receipt = verify_context_pack(
        db_path=str(db_path),
        capsule_path=capsule_path,
        spec_path=spec_path,
        context_path=context_path,
    )
    verify_path.write_bytes(canonical_bytes(verify_receipt))
    return db_path, capsule_path, spec_path, context_path, verify_path


def prepare_manifest(tmp_path: Path):
    _, capsule_path, spec_path, context_path, verify_path = make_inputs(tmp_path)
    ...
    return receipt, capsule_path, spec_path, context_path, verify_path, manifest_path


def test_prepare_binds_capsule_context_spec_and_checkpoint(tmp_path):
    receipt, capsule_path, spec_path, context_path, verify_path, manifest_path = prepare_manifest(tmp_path)
    ...
    assert manifest["artifact_binding"]["context_verification"]["sha256"] == sha(verify_path.read_bytes())
...
def test_prepare_is_byte_deterministic(tmp_path):
    ...
    prepare_session_input_manifest(
        capsule_path=capsule_path, context_path=context_path, spec_path=spec_path, context_verification_path=verify_path, output_path=one
    )
    prepare_session_input_manifest(
        capsule_path=capsule_path, context_path=context_path, spec_path=spec_path, context_verification_path=verify_path, output_path=two
    )
...
def test_context_verification_receipt_must_be_exact_pass(tmp_path):
    _, capsule_path, spec_path, context_path, verify_path = make_inputs(tmp_path)
    receipt = json.loads(verify_path.read_text("utf-8"))
    receipt["ok"] = False
    receipt["status"] = "OPERATIONAL_CONTEXT_VERIFY_FAIL"
    verify_path.write_bytes(canonical_bytes(receipt))
    with pytest.raises(SessionInputError, match="context_verification:NOT_EXACT_PASS"):
        prepare_session_input_manifest(
            capsule_path=capsule_path,
            context_path=context_path,
            spec_path=spec_path,
...
def test_context_verification_receipt_cannot_bind_other_context(tmp_path):
    _, capsule_path, spec_path, context_path, verify_path = make_inputs(tmp_path)
    receipt = json.loads(verify_path.read_text("utf-8"))
    receipt["context_file_sha256"] = "9" * 64
    receipt["expected_file_sha256"] = "9" * 64
    verify_path.write_bytes(canonical_bytes(receipt))
    with pytest.raises(SessionInputError, match="context_verification:NOT_EXACT_PASS"):
        prepare_session_input_manifest(
            capsule_path=capsule_path,
...
def test_manifest_extra_key_and_ceiling_escalation_rejected(tmp_path):
    _, _, _, _, _, manifest_path = prepare_manifest(tmp_path)
    manifest = json.loads(manifest_path.read_text("utf-8"))
    manifest["extra"] = True
...
def test_cli_prepare_and_verify(tmp_path, capsys):
    ...
    assert main([
        ...
        "--spec", str(spec_path),
        "--context-verification", str(verify_path),
