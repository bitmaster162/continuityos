...
def test_A06_tls_certificate_failure_hold_no_downgrade():
...
def test_A09_health_not_200_or_not_read_only_ready_hold():
    client, _ = consumer([FakeResponse(200, health_doc(status="DISABLED_DEFAULT", enabled=False))])
    with pytest.raises(c.ConsumerHold) as exc:
        client.health()
...
def test_A10_meta_source_role_disposition_currentness_or_authority_upgrade_mismatch_hold():
    for change in ({"stable_source_id": "wrong"}, {"role": "wrong"}, {"resolution_disposition": "wrong"}, {"currentness_status": "STALE"}, {"authority_upgraded": True}):
        client, _ = consumer([FakeResponse(200, health_doc(**change))])
        with pytest.raises(c.ConsumerHold) as exc:
            client.health()
...
def test_A11_record_projection_or_snapshot_digest_mismatch_hold():
    for key in ("record_digest", "projection_digest", "snapshot_digest"):
        client, _ = consumer([FakeResponse(200, health_doc(**{key: "0" * 64}))])
        with pytest.raises(c.ConsumerHold) as exc:
            client.health()
        assert exc.value.status == "HOLD_META_MISMATCH"
...
def test_A12_decisions_exact_139_contiguous_D001_D139_all_current():
    ...
    with pytest.raises(c.ConsumerHold): client.decisions()
...
def test_A13_projects_exact_10_unique_9_trunk_1_legacy_valid():
    ...
    with pytest.raises(c.ConsumerHold): client.projects()


def test_A14_health_decisions_projects_meta_identical():
    dc = decision_collection(); dc["meta"] = meta(record_digest="1" * 64)
    client, _ = consumer([FakeResponse(200, health_doc()), FakeResponse(200, dc), FakeResponse(200, project_collection())])
    with pytest.raises(c.ConsumerHold) as exc: client.snapshot()
    assert exc.value.status == "HOLD_META_MISMATCH"


def test_A15_cache_control_private_and_no_store_required():
    client, _ = consumer([FakeResponse(200, health_doc(), headers={"Cache-Control": "private"})])
    with pytest.raises(c.ConsumerHold) as exc: client.health()
    assert exc.value.status == "HOLD_CACHE_CONTROL"


def test_A16_D001_200_D139_200_D999_404_invalid_decision_422():
    client, _ = consumer([
        ...
        FakeResponse(422, {"detail": {"status": "INVALID_DECISION_ID"}}),
    ])
...
def test_A17_continuity_platform_200_unknown_project_404_invalid_project_422():
    record = {"project_id": "continuity-platform", "status": "CURRENT_TRUNK"}
    client, _ = consumer([
        FakeResponse(200, {"meta": meta(), "record": record}),
        FakeResponse(404, {"detail": {"status": "NOT_FOUND"}}),
        FakeResponse(422, {"detail": {"status": "INVALID_PROJECT_ID"}}),
    ])
...
def test_A18_outbound_http_method_allowlist_GET_only():
    client, factory = consumer([FakeResponse(200, health_doc())]); client.health()
    assert [item[0] for item in factory.requests] == ["GET"]


def test_A19_response_size_limit_fails_closed():
    client, _ = consumer([FakeResponse(200, health_doc(), headers={"Content-Length": str(c.HEALTH_MAX_BYTES + 1)})])
    with pytest.raises(c.ConsumerHold) as exc: client.health()
...
def test_A22_context_output_projection_only_with_all_effect_ceilings_deny():
    client, _ = consumer([FakeResponse(200, health_doc()), FakeResponse(200, decision_collection()), FakeResponse(200, project_collection())])
...
def test_A23_three_repeated_snapshots_identical_digests_and_canonical_output_hash():
    responses = []
    for _ in range(3): responses.extend([FakeResponse(200, health_doc()), FakeResponse(200, decision_collection()), FakeResponse(200, project_collection())])
    client, _ = consumer(responses); snapshots = [client.snapshot() for _ in range(3)]
    assert len({s["meta"]["snapshot_digest"] for s in snapshots}) == 1 and len({s["canonical_output_sha256"] for s in snapshots}) == 1


def test_A24_producer_503_hold_status_propagates_without_fallback():
    client, factory = consumer([FakeResponse(503, {"detail": {"status": "HOLD_PAYLOAD_RECONCILE", "reason": "remote"}})])
    with pytest.raises(c.ConsumerHold) as exc: client.health()
    assert exc.value.status == "HOLD_PAYLOAD_RECONCILE" and len(factory.requests) == 1
